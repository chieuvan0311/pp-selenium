    ...
    public partial class fTableManager : Form
    {
        ...
        public void Load_AccountTabale_GridView(string name, Table_Column_Status_Model st)
        {
            ...
            else
            {
                ...
                else if (name == "Hoạt động" || name == "Limit" || name == "180d" || name == "Set fail" || name == "Sai thông tin" || name == "Xác minh tk" || name == "Chưa set" || name == "Cần xóa")
                {
                    var listAcc = new On_Accounts_Dao().Get_List_On_Acc_By_AccType(name);
                    data = new Account_Table_Data().Get_Account_Table_Data(listAcc);
                }
                else if (name == "Đã fw email")
                {
                    PaypalDbContext db = new PaypalDbContext();
                    var listAcc = db.Accounts.Where(x => x.Set_Add_New_FwEmail == true).ToList();
                    data = new Account_Table_Data().Get_Account_Table_Data(listAcc);
                }
                else if (name == "Chưa fw email")
                {
                    PaypalDbContext db = new PaypalDbContext();
                    var listAcc = db.Accounts.Where(x => x.Set_Add_New_FwEmail != true).ToList();
                    data = new Account_Table_Data().Get_Account_Table_Data(listAcc);
                }
                else if (name == "Đã thêm email KP")
                {
                    PaypalDbContext db = new PaypalDbContext();
                    var listAcc = db.Accounts.Where(x => x.Set_Add_RecoveryEmail == true).ToList();
                    data = new Account_Table_Data().Get_Account_Table_Data(listAcc);
                }
                else if (name == "Chưa thêm email KP")
                {
                    PaypalDbContext db = new PaypalDbContext();
                    var listAcc = db.Accounts.Where(x => x.Set_Add_RecoveryEmail != true).ToList();
                    data = new Account_Table_Data().Get_Account_Table_Data(listAcc);
                }
                else if (name == "Đã đổi mk email")
                {
                    PaypalDbContext db = new PaypalDbContext();
                    var listAcc = db.Accounts.Where(x => x.Set_ChangedPassEmail == true).ToList();
                    data = new Account_Table_Data().Get_Account_Table_Data(listAcc);
                }
                else if (name == "Chưa đổi mk email")
                {
                    PaypalDbContext db = new PaypalDbContext();
                    var listAcc = db.Accounts.Where(x => x.Set_ChangedPassEmail != true).ToList();
                    data = new Account_Table_Data().Get_Account_Table_Data(listAcc);
                }
                else if (name == "Đã đổi mk PP")
                {
                    PaypalDbContext db = new PaypalDbContext();
                    var listAcc = db.Accounts.Where(x => x.Set_ChangedPassPP == true).ToList();
                    data = new Account_Table_Data().Get_Account_Table_Data(listAcc);
                }
                else if (name == "Chưa đổi mk PP")
                {
                    PaypalDbContext db = new PaypalDbContext();
                    var listAcc = db.Accounts.Where(x => x.Set_ChangedPassPP != true).ToList();
                    data = new Account_Table_Data().Get_Account_Table_Data(listAcc);
                }
                if (name == "Chọn danh mục") { tbCount.Text = null;  } else { tbCount.Text = data.Count.ToString(); }
            }
            ...
            else
            {
                ...
                gvAccountTable.Columns[38].Visible = false;
                gvAccountTable.Columns[39].Visible = false;
                if (status.Profile_Save == true) { gvAccountTable.Columns[40].Visible = true; } else { gvAccountTable.Columns[40].Visible = false; }
                gvAccountTable.Columns[41].Visible = false;
                gvAccountTable.Columns[42].Visible = false;
            }
        }
        private void Load_RightMouse_Click_Menu()
        {
            ...
            ToolStripItem update_accounts_info = menu.Items.Add("Cập nhật thông tin Acc");
            update_accounts_info.Click += new EventHandler(Update_Accounts_Info);
            // 
            ToolStripItem delete_accounts = menu.Items.Add("Admin - Xóa tài khoản SOS");
            delete_accounts.Click += new EventHandler(delete_account_BTN_Click);
            ...
            ToolStripItem remove_fwEmail = admin_fwEmail.DropDownItems.Add("Set - xóa Email Forward");
            remove_fwEmail.Click += new EventHandler(Remove_Fw_Email);
            ToolStripItem fwEmail_already_all = admin_fwEmail.DropDownItems.Add("Lọc - đã fw email");
            fwEmail_already_all.Click += new EventHandler(FwEmail_Already_All);
            ToolStripItem fwEmail_notyet_all = admin_fwEmail.DropDownItems.Add("Lọc - chưa fw email");
            fwEmail_notyet_all.Click += new EventHandler(FwEmail_Notyet_All);
            ...
            // Admin Add Recovery Email
            ToolStripMenuItem admin_recoEmail = (ToolStripMenuItem)menu.Items.Add("Admin - Set Email khôi phục");
            ToolStripItem add_reocoEmail = admin_recoEmail.DropDownItems.Add("Set - thêm Email KP beeliant");
            add_reocoEmail.Click += new EventHandler(Add_Recovery_Email);
            ToolStripItem added_recoEmail = admin_recoEmail.DropDownItems.Add("Lọc - đã thêm email KP");
            added_recoEmail.Click += new EventHandler(Show_Added_RecoveryEmail);  
            ToolStripItem no_recoEmail = admin_recoEmail.DropDownItems.Add("Lọc - chưa thêm email KP");
            no_recoEmail.Click += new EventHandler(Show_No_RecoveryEmail);
            // Change Password
            ToolStripMenuItem password_set = (ToolStripMenuItem)menu.Items.Add("Admin - Set đổi mật khẩu");
            ToolStripItem change_email_pass = password_set.DropDownItems.Add("Đổi mật khẩu email");
            change_email_pass.Click += new EventHandler(Change_Email_Password);
            ToolStripItem set_email_pass = password_set.DropDownItems.Add("Lọc - Đã đổi mk email");
            set_email_pass.Click += new EventHandler(Set_Email_Password);
            ToolStripItem unset_email_pass = password_set.DropDownItems.Add("Lọc - Chưa đổi mk emailP");
            unset_email_pass.Click += new EventHandler(Unset_Email_Password);
            //---pypal
            ToolStripItem change_papyal_pass = password_set.DropDownItems.Add("Đổi mật khẩu Paypal");
            change_papyal_pass.Click += new EventHandler(Change_Paypal_Password);
            ToolStripItem set_paypal_pass = password_set.DropDownItems.Add("Lọc - Đã đổi mk Paypal");
            set_paypal_pass.Click += new EventHandler(Set_Paypal_Password);
            ToolStripItem unset_paypal_pass = password_set.DropDownItems.Add("Lọc - Chưa đổi mk Paypal");
            unset_paypal_pass.Click += new EventHandler(Unset_Paypal_Password);
            //Admin setup only
                            ...
                            try
                            {
                                ...
                                if (next_step == true)
                                {
                                    ...
                                    if (result_02.Status == true)
                                    {
                                        ...
                                        new AccountDao().Update_Balance(Acc01.ID, balance, dTime);
                                        gvAccountTable.SelectedRows[i].Cells[7].Value = "Đăng nhập thành công--";
                                        gvAccountTable.SelectedRows[i].Cells[3].Value = balance;
                                        gvAccountTable.SelectedRows[i].Cells[8].Value = dTime;
                                    }
                                    else
                                    {
                                ...
                                if (chromeOFF_Box.Checked == true) { try { driver.Close(); } catch { } } else { if (i > 0) { driver.Manage().Window.Minimize(); } }
        ...
        // Admin update accounts info
        private void Update_Accounts_Info(object sender, EventArgs e)
        {
            fUpdateAccountsInfo form = new fUpdateAccountsInfo();
            this.Hide();
            form.ShowDialog();
            this.Show();
        }
                                    ...
                                    if (next_step == true && Acc.EmailType == "gmail.com")
                                    {
                                        ...
                                        if (result_02.Status == true)
                                        {
                                            ...
                                            if (result_07.Status == true)
                                            {
                                                ...
                                                gvAccountTable.SelectedRows[i].Cells[37].Value = "NO";
                                            }
                                            else
                                            {
        ...
        private void FwEmail_Already_All(object sender, EventArgs e)
        {
            cbAccStatusList.Text = "Đã fw email";
            Load_Form_TableManager();
        }
        private void FwEmail_Notyet_All(object sender, EventArgs e) 
        {
            cbAccStatusList.Text = "Chưa fw email";      
            Load_Form_TableManager();
        }
        // Set recovery email
                    ...
                    try
                    {
                        ...
                        for (int i = (selectedRowCount - 1); i >= 0; i--)
                        {
                            ...
                            try
                            {
                                ...
                                {
                                    if (Acc.Set_Add_RecoveryEmail != true)
                                    {
                                        bool next_step = false;
                                        var result_01 = new Create_Profiles().Create(Acc);
                                        driver = result_01.Driver;
                                        ...
                                        if (next_step == true)
                                        {
                                            ...
                                            if (result_02.Status == true)
                                            {
                                                var result_03 = new Add_RecoverEmail_Beeliant_Hotmail().Add(Acc, driver);
                                                driver = result_03.Driver;
                                                if (result_03.Status == true)
                                                {
                                                    gvAccountTable.SelectedRows[i].Cells[7].Value = "Đã thêm email khôi phục--";
                                                    gvAccountTable.SelectedRows[i].Cells[28].Value = result_03.Value_01;
                                                    gvAccountTable.SelectedRows[i].Cells[35].Value = "YES";
                                                }
                                                else
                                                {
                                                    gvAccountTable.SelectedRows[i].Cells[7].Value = "Thêm email khôi fail--";
                                                }
                                            }
                                            else
                                            {
        ...
        private void Show_Added_RecoveryEmail (object sender, EventArgs e)
        {
            cbAccStatusList.Text = "Đã thêm email KP";
            Load_Form_TableManager();
        }
        private void Show_No_RecoveryEmail(object sender, EventArgs e)
        {
            cbAccStatusList.Text = "Chưa thêm email KP";
            Load_Form_TableManager();
        }
        // Change Password
        ...
        private void Set_Email_Password(object sender, EventArgs e) 
        {
            cbAccStatusList.Text = "Đã đổi mk email";
            Load_Form_TableManager();
        }
        private void Unset_Email_Password(object sender, EventArgs e)
        {
            cbAccStatusList.Text = "Chưa đổi mk email";
            Load_Form_TableManager();
        }
        ...
        private void Set_Paypal_Password(object sender, EventArgs e)
        {
            cbAccStatusList.Text = "Đã đổi mk PP";
            Load_Form_TableManager();
        }
        private void Unset_Paypal_Password(object sender, EventArgs e)
        {
            cbAccStatusList.Text = "Chưa đổi mk PP";
            Load_Form_TableManager();
        }
        //Set others
